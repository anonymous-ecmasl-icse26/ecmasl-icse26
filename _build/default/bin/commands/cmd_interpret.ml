...
module InterpreterInstrument = struct
  ...
  let monitor (monitor_type : Monitor_type.t) =
    match monitor_type with
    | Nop -> (module Monitor.Nop : Monitor_intf.S)
    | Printer -> (module Monitor.Printer : Monitor_intf.S)
    | TaintTracker -> (module Monitor.TaintTracker (MultiLevel.M(SLevel.M)): Monitor_intf.S)
...
let interpret_esl code (entry : Interpreter.IEntry.t) (config : Options.config)
  ...
  let* p = Cmd_compile.compile code untyped advices file in
  interpret code entry config p
...
let run () (opts : Options.t) : unit Result.t =
  ...
  | Some ESL -> interpret_esl code entry opts.config opts.untyped opts.advices opts.input
