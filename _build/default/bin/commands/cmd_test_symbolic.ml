...
module Symbolic_engine =
  Ecma_sl_symbolic.Symbolic_engine.Make
    (Ecma_sl_symbolic.Symbolic_memory.Make
       (Ecma_sl_symbolic.Symbolic_object_default))
...
module Object = Ecma_sl_symbolic.Symbolic_object_default

let get_completion mem loc =
  match Symbolic_engine.Symbolic.Memory.get mem loc with
  | None -> Error "Leaked invalid location"
  | Some obj -> (
    match
      Object.get obj
        (Value.string "__completion__")
        (Obj.magic 0) Ecma_sl_symbolic.Symbolic_utils.true_
    with
    | [] -> Error "Object is not a completion"
    | _ ->
      let type_ =
        Object.get obj (Value.string "type") (Obj.magic 0)
          Ecma_sl_symbolic.Symbolic_utils.true_
        |> List.hd
        |> fst
      in
      let value =
        Object.get obj (Value.string "value") (Obj.magic 0)
          Ecma_sl_symbolic.Symbolic_utils.true_
        |> List.hd
        |> fst
      in
      let target =
        Object.get obj (Value.string "target") (Obj.magic 0)
          Ecma_sl_symbolic.Symbolic_utils.true_
        |> List.hd
        |> fst
      in
...
let check_result (test : Test.t) thread result =
  ...
  | Ok result -> (
    ...
    | List
      ...
      let mem = Symbolic_engine.Choice.mem thread in
...
let run_and_check_result i prelude results ({ Test.path; metadata; _ } as test)
  ...
  | Ok prog ->
    ...
    let () =
      ...
      try
        let _ =
          Symbolic_engine.run ~print_return_value:true ~no_stop_at_failure:false
            ~callback_out:(check_result test)
            ~callback_err:(fun _ _err -> [])
            ~solver_type:Smtml.Solver_type.Z3_solver path prog
        in
        ()
      with exn ->
  ...
  let results_str =
    Fmt.str
      "@\n\
       ...
       Failures: %d, Anomalies: %d, Skipped: %d, Timeout: %d"
      !succ !total
      (float !succ /. float !total *. 100.0)
      !time !fail !anomaly !skip !timeout
  in
