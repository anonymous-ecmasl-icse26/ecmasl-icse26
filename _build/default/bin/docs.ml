...
module SymbolicOpts = struct
  ...
  let objmodel =
    ...
    in
    let models = Arg.enum Enums.ObjModels.models in
    Arg.(
      value
      & opt models Enums.ObjModels.Default
      & info [ "objmodel"; "o" ] ~docv ~doc )

  let inputs =
    let doc =
      "Generate inputs for all paths. If not set, only witnesses for erroring \
       paths are generated."
    in
    Arg.(value & flag & info [ "all-inputs"; "A" ] ~doc)

  let continue : bool Term.t =
    let doc = "Do not stop on error. Continue processing other paths." in
    Arg.(value & flag & info [ "continue-on-error" ] ~doc)

  let solver : Smtml.Solver_type.t Term.t =
    let doc = "The underlying SMT solver." in
    Arg.(value & opt Smtml.Solver_type.conv Z3_solver & info [ "solver" ] ~doc)
end
