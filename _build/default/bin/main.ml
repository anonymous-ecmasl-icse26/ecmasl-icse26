...
let symbolic_term =
  ...
  and+ harness = Docs.ExecuteOpts.harness
  and+ objmodel = Docs.SymbolicOpts.objmodel
  and+ inputs = Docs.SymbolicOpts.inputs
  and+ continue = Docs.SymbolicOpts.continue
  and+ solver_type = Docs.SymbolicOpts.solver in
  Cmd_symbolic.run ~input ~lang ~target ~workspace ~harness ~objmodel ~inputs
    ~continue ~solver_type
...
let returncode =
  ...
  | Ok (`Ok (Error err)) -> begin
    ...
    | `Timeout -> Docs.ExitCodes.generic
  end
  | Error err -> begin
    match err with
