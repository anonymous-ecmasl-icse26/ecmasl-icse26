...
let create (section : string) : t =
  let (time, success, failure, anomaly, skipped, timeout) =
    (0.0, 0, 0, 0, 0, 0)
  in
  let items = Hashtbl.create !Base.default_hashtbl_sz in
  { section; time; success; failure; anomaly; skipped; timeout; items }
...
let rec count_item (item : item) : float * int * int * int * int * int =
  match item with
  | Test { time; result = Success; _ } -> (time, 1, 0, 0, 0, 0)
  | Test { time; result = Failure; _ } -> (time, 0, 1, 0, 0, 0)
  | Test { time; result = Anomaly; _ } -> (time, 0, 0, 1, 0, 0)
  | Test { result = Skipped; _ } -> (0.0, 0, 0, 0, 1, 0)
  | Test { result = Timeout; _ } -> (0.0, 0, 0, 0, 0, 1)
  | Tree tree ->
    count_results tree;
    ( tree.time
    , tree.success
...
and count_results (tree : t) : unit =
  let count_f tree _ item =
    let (time, success, failure, anomaly, skipped, timeout) = count_item item in
    ...
    tree.timeout <- tree.timeout + timeout
  in
...
let pp_total (ppf : Format.formatter) (tree : t) : unit =
  ...
  pf ppf "Failures: %d, Anomalies: %d, Skipped: %d, Timeout: %d" tree.failure
    tree.anomaly tree.skipped tree.timeout
