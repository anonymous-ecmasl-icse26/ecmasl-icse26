...
let init_filename (fn : Fpath.t) : unit = 
  filename := Some fn
...
module Impl = struct
  ...
  *)
  ...
  let open_in = function
    | Value.Str fpath -> Channel_utils.open_in channel_table fpath
    | v -> Fmt.failwith "invalid file path: %a" Value.pp v

  let open_out = function
    | Value.Str fpath -> Channel_utils.open_out channel_table fpath
    | v -> Fmt.failwith "invalid file path: %a" Value.pp v
  ...
  let input_line v =
    ...
    with
    | Ok v -> v
    | Error (`Failure msg) -> Fmt.failwith "%s" msg

  let input_all v =
    match
      let* ty = Channel_utils.find channel_table v in
      let* ic = Channel_utils.Type.get_in ty in
      Ok (Value.Str (In_channel.input_all ic))
    with
    | Ok v -> v
    | Error (`Failure msg) -> Fmt.failwith "%s" msg
  ...
  let output_string v str =
    ...
    match
      ...
      let* () =
        match str with
        | Value.Str str -> Ok (Out_channel.output_string oc str)
  ...
  let close v =
    let open Result in
    match Channel_utils.close channel_table v with
    | Ok () -> Value.undefined
    | Error (`Failure msg) -> Fmt.failwith "%s" msg
  ...
  let run_node_cmd (code : string) : int * string * string =
    let code' =
      String.map (fun c -> if Char.equal c '\"' then '\'' else c) code
    in
  ...
  let node_function (require' : Value.t) (path' : Value.t) (args' : Value.t) :
    ...
    let stdout' = String.remove_last stdout in
    let stderr' = String.remove_last stderr in
    Value.Str (String.cat stdout' stderr')
  ...
  let __dirname () =
    ...
    | None -> Log.fail "No filename"
    | Some fpath ->
      let (dirname, _) = Fpath.split_base fpath in
      Value.Str (Fpath.to_string dirname)
end
