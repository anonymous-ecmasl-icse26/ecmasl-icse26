...
module M (ITooling : Interpreter_tooling.M) = struct
  ...
  let initial_state (heap' : heap option) (itool : ITooling.t ref)
    ...
    { store; heap; stack; itool; time }

  let check_timeout (st : st) : unit =
    let time' = Unix.gettimeofday () in
    let diff = time' -. st.time in
    if Float.Infix.(diff > 300.0) then Runtime_error.(raise_notrace Timeout)
  ...
  let eval_small_step (p : Prog.t) (st : st) (s : Stmt.t) (cont : Stmt.t list) :
    ...
    let lbl_f v label =
      ITooling.Monitor.eval_small_step itool.mon label |> fun (_ : Value.t) -> v
    in
    let ( $$ ) v label = lbl_f v label in
    let lvl = Call_stack.depth st.stack - 1 in
    ITooling.Tracer.trace_stmt !(st.itool).code lvl s;
    ITooling.Profiler.count itool.pf `Stmt;
    match Stmt.view s with
    | Skip -> Intermediate (st, cont) $$ EmptyLab
    | Merge -> Intermediate (st, cont) $$ MergeLab
    | Debug s' ->
      ...
      Intermediate ({ st with store; heap; stack }, s' :: cont') $$ EmptyLab
    | Block ss -> Intermediate (st, ss @ cont) $$ EmptyLab
    | Print e ->
      Log.stdout "%a@." (print_pp st.heap) (eval_expr st e);
      Intermediate (st, cont) $$ PrintLab e
    | Return e -> (
      let v = eval_expr st e in
      ...
      match frame with
      | Call_stack.Toplevel _ -> Final v $$ ReturnLab e
      | Call_stack.Intermediate (_, restore) ->
        ...
        Intermediate (st', cont') $$ ReturnLab e )
    | Fail e ->
      let v = eval_expr st e in
      Error v $$ FailLab e
    | Assert e ->
      ...
      if v then Intermediate (st, cont) $$ AssertLab e
      else Error (assert_err e) $$ AssertLab e
    | Assign (x, e) ->
      let v = eval_expr st e in
      Store.set st.store x.it v;
      Intermediate (st, cont) $$ AssignLab (x.it, e)
    | AssignCall (x, fe, es) -> (
      let (fn, fvs) = eval_func_expr st fe in
      let vs = fvs @ List.map (eval_expr st) es in
      match ITooling.Monitor.interceptor x.it fn vs es with
      | Some lbl ->
        let value = ITooling.Monitor.eval_small_step itool.mon lbl in
        Store.set st.store x.it (List [ False; value ]);
      ...
      | None ->
        ...
        ITooling.Tracer.trace_call itool.code (lvl + 1) f s;
        Intermediate (st', cont'')
        $$ AssignCallLab (x.it, fn, fe, Func.params' f, es) )
    | AssignECall (x, fn, es) ->
      let vs = List.map (eval_expr st) es in
      let v = External.execute p st.store st.heap fn.it vs in
      Store.set st.store x.it v;
      Intermediate (st, cont) $$ AssignECallLab (x.it, fn.it, es, vs)
    | AssignNewObj x ->
      let loc = Loc.create () in
      let obj = Object.create () in
      Heap.set st.heap loc obj;
      Store.set st.store x.it (Value.loc loc);
      Intermediate (st, cont) $$ NewObjLab (x.it, loc)
    | AssignObjToList (x, e) ->
      let format_fld (fn, fv) = Value.List [ Str fn; fv ] in
      let (loc, obj) = eval_obj st e in
      let v = Value.List (Object.fld_lst obj |> List.map format_fld) in
      Store.set st.store x.it v;
      Intermediate (st, cont) $$ AssignObjToListLab (x.it, loc, e)
    | AssignObjFields (x, e) ->
      let format_fld (fn, _) = Value.Str fn in
      let (loc, obj) = eval_obj st e in
      let v = Value.List (Object.fld_lst obj |> List.map format_fld) in
      Store.set st.store x.it v;
      Intermediate (st, cont) $$ AssignObjFieldsLab (x.it, loc, e, v)
    | AssignInObjCheck (x, fe, oe) ->
      ...
      Store.set st.store x.it v;
      Intermediate (st, cont) $$ AssignInObjCheckLab (x.it, fn, loc, fe, oe)
    | FieldLookup (x, oe, fe) ->
      ...
      Store.set st.store x.it v;
      Intermediate (st, cont) $$ FieldLookupLab (x.it, loc, fn, oe, fe)
    | FieldAssign (oe, fe, e) ->
      let (loc, obj) = eval_obj st oe in
      let fn = eval_str st fe in
      let v = eval_expr st e in
      Object.set obj fn v;
      Intermediate (st, cont) $$ FieldAssignLab (loc, fn, oe, fe, e)
    | FieldDelete (oe, fe) ->
      let (loc, obj) = eval_obj st oe in
      let fn = eval_str st fe in
      Object.delete obj fn;
      Intermediate (st, cont) $$ FieldDeleteLab (loc, fn, oe, fe)
    | If (e, s1, s2) -> (
      let v = eval_bool st e in
      ...
      | (true, Block ss, _) | (false, _, Block ss) ->
        ...
        Intermediate (st, cont') $$ BranchLab e
      | (false, _, Skip) -> Intermediate (st, cont) $$ BranchLab e
    ...
    | While (e, s') ->
      ...
      Intermediate (st, loop :: cont) $$ BranchLab e
    | Switch (e, css, dflt) -> (
      let v = eval_expr st e in
      ...
      | (Some { it = Block ss; _ }, _) | (None, Some { it = Block ss; _ }) ->
        let cont' = ss @ ((Stmt.Merge @?> none) :: cont) in
        Intermediate (st, cont') $$ BranchLab e
      | (None, None) -> Intermediate (st, cont) $$ BranchLab e
      | (Some s1', _) ->
