...
module type S = sig
  ...
  val create_object : unit -> object_

  val to_list_object : object_ -> (value * value) list

  val get_fields_object : object_ -> value list
  ...
  val get_field :
    t -> Loc.t -> value -> Solver.t -> value -> (value * value list) list

  val has_field : t -> Loc.t -> value -> value

  val delete_field :
    t -> Loc.t -> value -> Solver.t -> value -> (t * value list) list

  val pp : t Fmt.t

  val loc : value -> (Loc.t option * value list) list
