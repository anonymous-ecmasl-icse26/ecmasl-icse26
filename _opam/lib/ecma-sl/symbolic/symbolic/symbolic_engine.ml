...
module Make
    (Memory : Memory_intf.S with type value = Symbolic_value.value)
    (Failure : sig
      type t
    ...
    () =
struct
  module Symbolic = Symbolic.Make (Memory)
  module Interpreter = Interpreter_functor.Make (Symbolic) ()
  module Symbolic_esl_ffi = Symbolic_esl_ffi.Make (Symbolic)
  ...
  let check_return_value ~print_return_value thread = function
    | Ok result -> (
      ...
      | List [ Hc.{ node = Val False; _ }; result ] ->
        let mem = Symbolic.Choice.mem thread in
  ...
  let run ?timeout ?(lazy_values = true) ?(print_return_value = true)
    ?(no_stop_at_failure = false) ?(target = "main") ~solver_type ~callback_out
    ~callback_err filename prog =
    Logs.debug (fun k -> k "using solver: %a" Smtml.Solver_type.pp solver_type);
    ...
    let thread = Symbolic.Choice.create ?timeout solver_type in
      ...
      let () =
        try
          results (fun (result, thread) ->
            let result' =
              check_return_value ~print_return_value thread result
            in
            match result' with
            | Ok () ->
              let _ = callback_out thread result in
              ()
            | Error witness ->
        ...
        with Exit -> ()
      in
      let solver = Symbolic.Choice.solver thread in
      report.execution_time <- Unix.gettimeofday () -. report.execution_time;
      report.solver_time <- Solver.solver_time solver;
      report.solver_queries <- Solver.solver_count solver;
