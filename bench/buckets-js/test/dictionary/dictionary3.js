...
esl_symbolic.assert(esl_symbolic.eq(l1, 2));
var t1 = keys[0];
var t2 = keys[1];
...
esl_symbolic.assert(
    esl_symbolic.or(
        esl_symbolic.and(
            esl_symbolic.eq(t1, s1),
            esl_symbolic.eq(t2, s2)
        ),
        esl_symbolic.and(
            esl_symbolic.eq(t1, s2),
            esl_symbolic.eq(t2, s1)
        )
    )
);
...
esl_symbolic.assert(esl_symbolic.eq(l2, 2));
var y1 = values[0];
var y2 = values[1];
...
esl_symbolic.assert(
    esl_symbolic.or(
        esl_symbolic.and(
            esl_symbolic.eq(y1, x1),
            esl_symbolic.eq(y2, x2)
        ),
        esl_symbolic.and(
            esl_symbolic.eq(y1, x2),
            esl_symbolic.eq(y2, x1)
        )
    )
);
