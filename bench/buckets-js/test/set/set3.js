...
esl_symbolic.assert(
    esl_symbolic.and(
        esl_symbolic.or(
            esl_symbolic.and(
                esl_symbolic.and(
                    esl_symbolic.or(esl_symbolic.eq(x3, x1), esl_symbolic.eq(x3, x2)),
                    esl_symbolic.or(esl_symbolic.eq(x4, x1), esl_symbolic.eq(x4, x2))
                ),
                res1
            ),
            esl_symbolic.and(
                esl_symbolic.or(
                    esl_symbolic.and(esl_symbolic.neq(x3, x1), esl_symbolic.neq(x3, x2)),
                    esl_symbolic.and(esl_symbolic.neq(x4, x1), esl_symbolic.neq(x4, x2))
                ),
                esl_symbolic.not(res1)
            )
        ),
        esl_symbolic.and(res2, res3)
    )
);
