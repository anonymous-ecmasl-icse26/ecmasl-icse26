...
let compile_pipeline code (untyped : bool) (advices : Fpath.t list)
  (fname : string) (fpath : string) : Prog.t Result.t =
  EParsing.load_file code ~file:fname fpath
...
let compile code (untyped : bool) (advices : Fpath.t list) (file : Fpath.t) :
  Prog.t Result.t =
  Result.esl_exec code @@ fun () ->
  let (fname, fpath) = (Fpath.filename file, Fpath.to_string file) in
  let* p = compile_pipeline code untyped advices fname fpath in
...
let run () (opts : Options.t) : unit Result.t =
  ...
  let* p = compile code opts.untyped opts.advices opts.input in
