...
let setup_program_harness code (interp : Prog.t) (harness : Fpath.t) :
  ...
  External.init_filename harness;
...
let setup_execution code (untyped : bool) (advices : Fpath.t list)
  (jsinterp : Enums.JSInterp.t) (harness : Fpath.t option) :
  (Prog.t * Value.t Heap.t option) Result.t =
  let finterp = Enums.JSInterp.interp jsinterp in
  let* interp = Cmd_compile.compile code untyped advices (Fpath.v finterp) in
  match harness with
  | None -> Ok (interp, None)
  | Some harness' ->
...
let run () (opts : Options.t) : unit Result.t =
  ...
  External.init_filename opts.input;
  let* setup =
    setup_execution code true opts.advices opts.jsinterp opts.harness
  in
