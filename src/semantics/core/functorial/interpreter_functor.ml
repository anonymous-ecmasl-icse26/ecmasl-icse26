...
module Make (P : Interpreter_functor_intf.P) () :
  ...
  let exec_stmt stmt (state : State.exec_state) : State.stmt_result Choice.t =
    ...
    let ( let*/ ) o f =
      match o with Error e -> error (`Failure e) | Ok o -> f o
    in
    let* m = Choice.get_memory () in
    ...
    | AssignInObjCheck (x, e_field, e_loc) ->
      ...
      let loc_value = eval_expr locals e_loc in
      let* loc = Memory.loc loc_value in
      let*/ loc' = loc in
      ...
      ok { state with locals = Store.add_exn locals x.it v }
    | AssignObjToList (x, e) -> (
      let loc_value = eval_expr locals e in
      let* loc = Memory.loc loc_value in
      let*/ loc' = loc in
      let* heap = Choice.get_memory () in
      ...
      | Some o ->
        let v =
          Value.mk_list (List.map Value.mk_tuple (Memory.to_list_obj o))
        in
        ok { state with locals = Store.add_exn locals x.it v } )
    | AssignObjFields (x, e) -> (
      let loc_value = eval_expr locals e in
      ...
      | None -> error (`Failure (Fmt.str "'%a' not found in heap" Loc.pp loc'))
      | Some o ->
        let v = Value.mk_list @@ Memory.fields_of_obj o in
        ok { state with locals = Store.add_exn locals x.it v } )
    | FieldAssign (e_loc, e_field, e_v) ->
      ...
      let*/ loc' = loc in
      let* heap = Choice.get_memory () in
      let* _ = Memory.set_field heap loc' ~field ~data:v in
      ok state
    | FieldDelete (e_loc, e_field) ->
      ...
      let*/ loc' = loc in
      let* heap = Choice.get_memory () in
      let* _ = Memory.delete_field heap loc' field in
      ok state
