...
module Make
  ...
  Intf
    with type extern_func = Symbolic.Extern_func.extern_func
     and type env_t = Symbolic.env = struct
  module Choice = Symbolic.Choice
  module Extern_func = Symbolic.Extern_func
  module Env = Symbolic.Env
  open Extern_func

  type env_t = Symbolic.env
  ...
  let concrete_api =
    ...
    let open_in v =
      match Smtml.Expr.view v with
      | Val (Str fpath) -> ok_v @@ Channel_utils.open_in channel_table fpath
      | _ -> failure "invalid file path: %a" Smtml.Expr.pp v
    in
    let open_out v =
      match Smtml.Expr.view v with
      | Val (Str fpath) -> ok_v @@ Channel_utils.open_out channel_table fpath
      | _ -> failure "invalid file path: %a" Smtml.Expr.pp v
    in
    let input_line v =
      let open Result in
  ...
  let symbolic_api filename =
    ...
    let assume (e : value) =
      ...
      | _ ->
        Choice.with_mutable_thread ~check_sat:true @@ fun thread ->
        (Ok (Symbolic_value.mk_symbol "undefined"), Choice.add_pc thread e)
    in
    let evaluate (e : value) =
      Choice.with_thread @@ fun thread ->
      let pc = Smtml.Expr.Set.add e @@ Choice.pc thread in
      let solver = Choice.solver thread in
      assert (Choice.is_sat @@ Solver.check_set solver pc);
    ...
    let maximize (e : value) =
      Choice.with_thread @@ fun thread ->
      let pc = Smtml.Expr.Set.to_list @@ Choice.pc thread in
      let opt = Choice.optimizer thread in
    ...
    let str_split (s : Symbolic_value.value) (r : Symbolic_value.value) =
      ...
      (Ok result, Choice.add_pc thread cond)
    in
    let str_match (s : Symbolic_value.value) =
      ...
      (Ok (Symbolic_value.mk_list [ x ]), Choice.add_pc thread cond)
    in
    let dirname () =
    ...
    let process_cwd () =
      ...
      | Error (`Msg err) ->
        ...
        ok @@ Symbolic_value.mk_symbol "undefined"
    in
    let ite c e1 e2 = ok Smtml.Expr.(Bool.ite c e1 e2) in
    let idx_symbol expr =
      (* Fmt.pr "idx_symbol: %a @." Smtml.Expr.pp expr; *)
      match Symbolic_utils.idx_symbol expr with
      | Ok s -> ok s
      | Error _ -> failure "%s: invalid argument" __FUNCTION__
    in
    let get_idx expr =
      (* Fmt.pr "get_idx: %a @." Smtml.Expr.pp expr; *)
      match Symbolic_utils.get_idx expr with
      | Ok i -> ok i
      | _ -> failure "%s: invalid argument" __FUNCTION__
    in
    let is_idx_symbol expr =
      (* Fmt.pr "is_idx_symbol: %a @." Smtml.Expr.pp expr; *)
      match Symbolic_utils.is_idx_symbol expr with
      | true -> ok_v True
      | false -> ok_v False
    in
    let are_lzs_on () = if !allow_lazy_values then ok_v True else ok_v False in
    of_array
      [| ("ite", Extern_func (Func (Arg (Arg (Arg Res))), ite))
       ; ("idx_symbol", Extern_func (Func (Arg Res), idx_symbol))
       ; ("get_idx", Extern_func (Func (Arg Res), get_idx))
       ; ("is_idx_symbol", Extern_func (Func (Arg Res), is_idx_symbol))
       ...
       ; ("is_sat_branch", Extern_func (Func (Arg Res), is_sat_branch))
       ; ("is_sat_check", Extern_func (Func (Arg Res), is_sat_check))
